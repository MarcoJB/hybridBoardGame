...
import * as Peer from "./simplepeer";
...
export class AppComponent implements OnInit {
  ...
  connectSocketServer(): void {
    ...
    this.socket = new WebSocket("wss://0a46125f5c1f.ngrok.io");
      ...
      switch (message.type) {
        case "INIT":
          ...
          for (const client of message.data.clients) {
            this.peers[client] = new Peer({initiator: true, stream: this.unescapedVideo.captureStream()});
        ...
        case "OFFER":
          if (!this.peers.hasOwnProperty(message.from)) {
            this.peers[message.from] = new Peer({stream: this.unescapedVideo.captureStream(10)});
